...
public class DatabaseDocTruyen extends SQLiteOpenHelper {
    ...
    // Chú ý phân quyền 1.admin 2.người dùng
    private String SQLQuery2 = "INSERT INTO TaiKhoan VAlUES (null,'admin','admin','admin@gmail.com',2)";
    private String SQLQuery3 = "INSERT INTO TaiKhoan VAlUES (null,'thuy','thuy','thuy@gmail.com',1)";
...
//
    ...
    @SuppressLint("Range")
    public TaiKhoan getTaiKhoanByTaiKhoan(String taiKhoan) {
        SQLiteDatabase db = this.getReadableDatabase();
        TaiKhoan taiKhoanInfo = null;
        Cursor cursor = null;

        try {
            String selectQuery = "SELECT * FROM " + TABLE_TAIKHOAN + " WHERE " + TEN_TAI_KHOAN + " = ?";
            cursor = db.rawQuery(selectQuery, new String[]{taiKhoan});

            if (cursor != null && cursor.moveToFirst()) {
                // Lấy thông tin tài khoản từ cột trong bảng và tạo đối tượng TaiKhoan mới
                taiKhoanInfo = new TaiKhoan(
                        cursor.getString(cursor.getColumnIndex(TEN_TAI_KHOAN)),
                        cursor.getString(cursor.getColumnIndex(MAT_KHAU)),
                        cursor.getString(cursor.getColumnIndex(EMAIL)),
                        cursor.getInt(cursor.getColumnIndex(PHAN_QUYEN))
                );
                taiKhoanInfo.setmId(cursor.getInt(cursor.getColumnIndex(ID_TAI_KHOAN))); // Đặt ID của tài khoản
            }
        } catch (Exception e) {
            Log.e("DatabaseHelper", "Lỗi khi lấy thông tin tài khoản từ cơ sở dữ liệu: " + e.getMessage());
        } finally {
            if (cursor != null) {
                cursor.close();
            }
